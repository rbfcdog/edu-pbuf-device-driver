...
/*
 * edu_pbuf - educational alert-queue pseudo character driver.
 *
...
MODULE_PARM_DESC(capacity, "Allocated alert queue capacity in bytes");
...
/* Serializes pending alerts, metadata, flags and statistics. */
...
static int __init edu_pbuf_init(void)
{
	...
	/* Create /dev/edu_pbuf automatically for the alert queue demo. */
...
MODULE_DESCRIPTION("Educational system alert queue character driver");
MODULE_LICENSE("GPL");
